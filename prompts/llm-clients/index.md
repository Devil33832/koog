...
You can request multiple alternative responses from the model in a single call by using the `executeMultipleChoices()` method. It requires additionally specifying the [`numberOfChoices`](../structured-prompts/#prompt-parameters) LLM parameter in the prompt being executed.
...
You can also request multiple choices by adding the [`numberOfChoices`](../structured-prompts/#prompt-parameters) LLM parameter into the prompt.
